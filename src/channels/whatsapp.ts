...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // Pending questions: chatJid → { questionId, options, poll? }
    // Vote arrives either as a poll tap (decrypted via pollEncKey) or as a
    // /slash text reply (kept for fallback when poll send fails).
    const pendingQuestions = new Map<
      string,
      {
        ...
        poll?: {
          msgId: string;
          encKey: Uint8Array;
          optionHashByHex: Map<string, NormalizedOption>;
          creationKey: WAMessageKey;
        };
      }
    >();
    // Reverse index for poll vote lookup: pollMsgId → chatJid
    const pollIndex = new Map<string, string>();
      ...
      sock.ev.on('messages.upsert', async ({ messages }) => {
        for (const msg of messages) {
          try {
            ...
            // Poll vote → decrypt with the secret we stashed at send time,
            // then map the SHA-256 option hash back to the original option.
            const pollUpdate = normalized.pollUpdateMessage;
            if (pollUpdate?.pollCreationMessageKey?.id && pollUpdate.vote) {
              const pollMsgId = pollUpdate.pollCreationMessageKey.id;
              const chatJidForPoll = pollIndex.get(pollMsgId);
              const pending = chatJidForPoll ? pendingQuestions.get(chatJidForPoll) : undefined;
              if (pending?.poll && sock.user) {
                try {
                  // Baileys' getKeyAuthor returns meId for fromMe messages, but
                  // self-chat polls can be encoded with either the phone JID or
                  // the LID identity depending on addressingMode at send/vote
                  // time. WhatsApp's AAD must match exactly, so try both
                  // identities for creator and voter (4 combinations) until GCM
                  // auth succeeds.
                  const mePhone = jidNormalizedUser(sock.user.id);
                  const meLid = sock.user.lid ? jidNormalizedUser(sock.user.lid) : undefined;
                  const creatorCandidates = Array.from(
                    new Set(
                      [
                        getKeyAuthor(pending.poll.creationKey, mePhone),
                        meLid ? getKeyAuthor(pending.poll.creationKey, meLid) : undefined,
                      ].filter((v): v is string => !!v),
                    ),
                  );
                  const voterCandidates = Array.from(
                    new Set(
                      [getKeyAuthor(msg.key, mePhone), meLid ? getKeyAuthor(msg.key, meLid) : undefined].filter(
                        (v): v is string => !!v,
                      ),
                    ),
                  );
                  let voteMsg: ReturnType<typeof decryptPollVote> | undefined;
                  let usedVoter: string | undefined;
                  let lastErr: unknown;
                  for (const c of creatorCandidates) {
                    for (const v of voterCandidates) {
                      try {
                        voteMsg = decryptPollVote(
                          ...
                          {
                            pollEncKey: pending.poll.encKey,
                            pollCreatorJid: c,
                            pollMsgId,
                            voterJid: v,
                          },
                  ...
                  if (!voteMsg) throw lastErr ?? new Error('All JID combos failed');
                  const voterJid = usedVoter!;
                  const selected = (voteMsg.selectedOptions || []).map((b) => Buffer.from(b).toString('hex'));
                  const matched = selected
                    .map((h) => pending.poll!.optionHashByHex.get(h))
                    .find((o): o is NormalizedOption => !!o);
                  if (matched) {
                    // Downstream wants a phone JID — translate LID if needed.
                    const voterPhoneJid = await translateJid(voterJid, msg.key.remoteJidAlt ?? undefined);
                    const voterName = msg.pushName || voterPhoneJid.split('@')[0];
                    setupConfig.onAction(pending.questionId, matched.value, voterPhoneJid);
                    pollIndex.delete(pollMsgId);
                    pendingQuestions.delete(chatJidForPoll!);
                    await sendRawMessage(chatJidForPoll!, `${matched.selectedLabel} by ${voterName}`);
                    log.info('Poll question answered', {
                      questionId: pending.questionId,
                      value: matched.value,
                      voterName,
                    });
                  } else if (selected.length === 0) {
                    // Empty selectedOptions = user un-voted; leave pending.
                    log.debug('Poll vote cleared', { pollMsgId });
                  } else {
                    log.warn('Poll vote did not match any known option', { pollMsgId });
                  }
                } catch (err) {
                  log.warn('Failed to decrypt poll vote', { pollMsgId, err });
                }
              }
              continue;
            }
            ...
            if (pending && content.startsWith('/')) {
              ...
              if (matched) {
                ...
                if (pending.poll) pollIndex.delete(pending.poll.msgId);
    ...
    const adapter: ChannelAdapter = {
      ...
      async deliver(
        ...
        // Ask question → WhatsApp poll (single-select) when ≤12 options;
        // longer lists fall back to a text message with /slash replies.
        if (content.type === 'ask_question' && content.questionId && content.options) {
          ...
          // Drop any earlier pending poll for this chat — keeps pollIndex tidy.
          const stale = pendingQuestions.get(platformId);
          if (stale?.poll) pollIndex.delete(stale.poll.msgId);

          // Poll path: name is short (we pack title+question, WhatsApp caps poll
          // name length). Single-select keeps semantics of "tap once to answer."
          const canUsePoll = options.length <= 12;
          if (canUsePoll) {
            try {
              const encKey = crypto.randomBytes(32);
              const pollName = question ? `${title}\n\n${question}` : title;
              ...
              if (pollMsgId && sent?.key) {
                const optionHashByHex = new Map<string, NormalizedOption>();
                for (const o of options) {
                  const hex = crypto.createHash('sha256').update(o.label).digest('hex');
                  optionHashByHex.set(hex, o);
                }
                pendingQuestions.set(platformId, {
                  questionId,
                  options,
                  poll: { msgId: pollMsgId, encKey, optionHashByHex, creationKey: sent.key },
                });
                pollIndex.set(pollMsgId, platformId);
                if (pendingQuestions.size > PENDING_QUESTIONS_MAX) {
                  const oldestJid = pendingQuestions.keys().next().value!;
                  const oldest = pendingQuestions.get(oldestJid);
                  if (oldest?.poll) pollIndex.delete(oldest.poll.msgId);
                  pendingQuestions.delete(oldestJid);
                }
                sentMessageCache.set(pollMsgId, sent.message!);
                return pollMsgId;
              }
              log.warn('Poll send returned no message id — falling back to text', { questionId });
            } catch (err) {
              log.warn('Poll send failed — falling back to text', { questionId, err });
            }
          }

          // Fallback: text + /slash command replies (also used when options >12).
